...
export default async function Home() {
  ...
  return <>
    ...
    <main>
      ...
      <section className="section container" id="projects"><SectionHeading index="02" title="Featured Projects" action={<Link className="text-link" href="/projects" prefetch={false}>View all projects <Icon name="arrow" size={16} /></Link>} /><div className="project-grid">{githubRepositories.length ? githubRepositories.slice(0, 3).map((repo) => <GitHubProjectCard key={repo.name} repo={repo} />) : featuredProjects.map((project) => <Link className="project-card" href={`/projects/${project.slug}`} prefetch={false} key={project.slug}><ProjectPreview project={project} /><div className="project-card__body"><div><h3>{project.title}</h3><p>{project.description}</p></div><span className="project-arrow"><Icon name="arrow" size={18} /></span><div className="tag-row">{project.technologies.map((tech) => <span key={tech}>{tech}</span>)}</div></div></Link>)}</div></section>
...
function GitHubProjectCard({ repo }: { repo: GitHubRepository }) {
  return <a className="project-card" href={repo.htmlUrl} target="_blank" rel="noreferrer"><ProjectPreview project={{ slug: "github-repository", type: "GitHub repository" }} /><div className="project-card__body"><div><h3>{repo.name}</h3><p>{repo.description}</p></div><span className="project-arrow"><Icon name="arrow" size={18} /></span><div className="tag-row"><span>{repo.language}</span><span>★ {repo.stars}</span></div></div></a>;
}
