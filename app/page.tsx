...
export default async function Home() {
  ...
  return <>
    ...
    <main>
      ...
      <section className="section container" id="projects"><SectionHeading index="02" title="Featured Projects" action={<Link className="text-link" href="/projects">View all projects <Icon name="arrow" size={16} /></Link>} /><div className="project-grid">{githubRepositories.length ? githubRepositories.slice(0, 3).map((repo) => <GitHubProjectCard key={repo.name} repo={repo} />) : featuredProjects.map((project) => <Link className="project-card" href={`/projects/${project.slug}`} key={project.slug}><ProjectPreview project={project} /><div className="project-card__body"><div><h3>{project.title}</h3><p>{project.description}</p></div><span className="project-arrow"><Icon name="arrow" size={18} /></span><div className="tag-row">{project.technologies.map((tech) => <span key={tech}>{tech}</span>)}</div></div></Link>)}</div></section>
