...
export function BrandMark() {
  return <span className="brand-mark" aria-label={`${profile.name} home`}><span className="brand-bracket">&lt;</span><Icon name="code" size={19} /><span className="brand-bracket">/&gt;</span></span>;
}

export function SiteHeader({ active = "About" }: { active?: string }) {
  return <header className="site-header"><div className="site-header__inner container"><Link href="/" className="brand-link" aria-label="Home"><BrandMark /></Link><nav className="desktop-nav" aria-label="Primary navigation">{["About", "Experience", "Projects", "Skills", "Contact"].map((item) => <a key={item} className={active === item ? "is-active" : ""} href={item === "About" ? "/#about" : `/#${item.toLowerCase()}`}>{item}</a>)}<a href="/resume" className="nav-resume">Resume <span aria-hidden="true">↓</span></a></nav><div className="header-actions"><button className="theme-control" type="button" aria-label="Toggle theme" data-theme-toggle><Icon name="sun" size={15} /><span className="theme-switch"><span /></span><Icon name="moon" size={15} /></button><button className="mobile-menu-button" type="button" aria-label="Open menu" aria-expanded="false" data-menu-toggle><Icon name="menu" /></button></div></div><div className="mobile-nav" data-mobile-nav>{["About", "Experience", "Projects", "Skills", "Contact"].map((item) => <a key={item} href={item === "About" ? "/#about" : `/#${item.toLowerCase()}`}>{item}</a>)}<a href="/resume">Quick Resume <span aria-hidden="true">↓</span></a></div></header>;
}
...
export function ProjectPreview({ project, compact = false }: { project: ProjectItem; compact?: boolean }) {
