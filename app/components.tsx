...
export async function SiteHeader({ active = "About" }: { active?: string }) {
  ...
  const navItems = [{ label: "About", icon: "mark" }, ...(currentProfile.experience.length ? [{ label: "Experience", icon: "code" }] : []), { label: "Projects", icon: "mark" }, { label: "Skills", icon: "code" }, { label: "Contact", icon: "mail" }];
  return <header className="site-header"><div className="site-header__inner container"><Link href="/" prefetch={false} className="brand-link" aria-label="Home"><BrandMark currentProfile={currentProfile} /></Link><nav className="desktop-nav" aria-label="Primary navigation">{navItems.map(({ label }) => <a key={label} className={active === label ? "is-active" : ""} aria-current={active === label ? "location" : undefined} data-section-nav={label.toLowerCase()} href={label === "About" ? "/#about" : `/#${label.toLowerCase()}`}>{label}</a>)}<a href="/resume" className="nav-resume" aria-current={active === "Resume" ? "page" : undefined}>Resume <span aria-hidden="true">↗</span></a></nav><div className="header-actions"><button className="theme-control" type="button" aria-label="Toggle theme" aria-pressed="false" data-theme-toggle><Icon name="sun" size={15} /><span className="theme-switch"><span /></span><Icon name="moon" size={15} /></button><button className="mobile-menu-button" type="button" aria-label="Open menu" aria-expanded="false" aria-controls="mobile-navigation" data-menu-toggle><span className="menu-icon"><Icon name="menu" /><Icon name="close" /></span></button></div></div><nav className="mobile-nav" id="mobile-navigation" aria-label="Mobile navigation" data-mobile-nav>{navItems.map(({ label, icon }) => <a key={label} data-section-nav={label.toLowerCase()} href={label === "About" ? "/#about" : `/#${label.toLowerCase()}`}><span className="mobile-nav__icon"><Icon name={icon} size={21} /></span>{label}<Icon name="arrow" size={19} /></a>)}<a href="/resume"><span className="mobile-nav__icon"><Icon name="mark" size={21} /></span>Quick Resume <span aria-hidden="true">↗</span></a><button className="mobile-theme-control" type="button" aria-label="Toggle theme" aria-pressed="false" data-theme-toggle><Icon name="sun" size={19} /><span>Appearance</span><span className="theme-switch"><span /></span><Icon name="moon" size={19} /></button></nav></header>;
}
...
export function ProjectPreview({ project, compact = false }: { project: Pick<ProjectItem, "slug" | "type" | "coverImage">; compact?: boolean }) {
  if (project.coverImage) return <div className={`project-visual project-visual--cover ${compact ? "project-visual--compact" : ""}`}><img src={project.coverImage} width="1200" height="780" loading="lazy" decoding="async" alt={`${project.type} cover`} /></div>;
  return <div className={`project-visual project-visual--${project.slug} ${compact ? "project-visual--compact" : ""}`} aria-hidden="true"><div className="mock-window-bar"><span /><span /><span /><em>{project.type}</em></div><div className="mock-window-content"><div className="mock-sidebar"><span /><span /><span /><span /></div><div className="mock-main"><div className="mock-heading" /><div className="mock-row"><span /><span /><span /></div><div className="mock-chart"><i /><i /><i /><i /><i /></div><div className="mock-row mock-row--short"><span /><span /></div></div></div></div>;
}
