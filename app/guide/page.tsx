...
export default function GuidePage() {
  return <><SiteHeader /><main className="guide-page"><div className="container guide-intro"><span className="section-index">PRIVATE MAINTENANCE GUIDE</span><h1>Keep the site easy to update.</h1><p>This page is intentionally not in the public navigation. Every personal detail is kept in one data file so you can return months later and know exactly where to edit.</p><Link className="text-link" href="/" prefetch={false}>← Back to portfolio</Link></div><section className="container guide-list">{guideItems.map(([title, text], index) => <article key={title}><span className="guide-number">0{index + 1}</span><div><h2>{title}</h2><p>{text}</p></div></article>)}</section></main><SiteFooter /></>;
}
